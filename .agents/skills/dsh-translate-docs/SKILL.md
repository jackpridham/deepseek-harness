...
description: Optionally run the DeepSeek Harness translation workflow when the user explicitly requests a Chinese or English companion document, including generated briefings, delegated prose translation, whole-document translation, and scoped consistency checks.
...
Run this workflow only when the user explicitly invokes `dsh-translate-docs` by name. Never select or load it for ordinary documentation work, from another skill, because an English document changed, or from an inferred translation need. The repository does not require companion translations.

## What this skill is

**This skill is guidance, not a translation memory.** It is the optional workflow map for creating or maintaining a specifically requested `foo.md ↔ foo.zh.md` pair. English remains the maintained repository documentation source outside the requested translation scope. You are the translator: the rules below say what must hold for the requested pair, not how to phrase any particular sentence — phrasing judgment is yours, terminology is not.
...
Frozen Agent Notes under `.agents/notes/archived/` are not translation work. Their existing artifacts are sealed by the archive verifier; never update, re-record, or repair either side after archival.
...
5. **Record and verify, scoped**: `pnpm run verify-translation-pairing --write <pair>` then `pnpm run verify-translation-pairing <pair>`. These commands validate only the explicitly requested optional pair; they are not part of ordinary `doc-sync` or CI.
...
- `pnpm run verify-translation-pairing --list` reports legacy pairing state for translation work. Missing and out-of-sync rows are informational outside an explicitly requested translation task.
- `pnpm run gen-translation-brief` with no arguments prints the briefing for every out-of-sync pair.
- In explicitly requested translation work, keep the named pair internally consistent. Unrelated English documentation changes do not enter the work list.
...
3. The manifest is legacy scope metadata for optional tooling; do not add an English document to it merely to avoid creating a translation.
4. Before the PR: explicitly requested pairs are green under the scoped check; `pnpm run doc-sync` runs once at PR level per [dsh-pre-push-checks](../dsh-pre-push-checks/SKILL.md), not inside each translation task.
