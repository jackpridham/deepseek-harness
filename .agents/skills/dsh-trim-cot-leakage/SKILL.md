...
3. Fix owner-first per surface: generated catalogs → fix the source JSDoc or generator template, then regenerate the English artifact; type-equivalence fences → fix the source JSDoc, then re-paste the maintained English page (`verify-type-equiv` pins it); existing translations stay untouched unless the user explicitly includes them; model-visible strings → wording is behavior, so flag for a snapshot-backed change instead of silently rewording.
...
5. Verify: re-run the batteries expecting only sanctioned keeps, this skill's own directory, and the owning note's quoted evidence; confirm every remaining citation resolves at HEAD; run the gates for touched surfaces (`doc-sync` for docs and `verify-type-equiv` for maintained type projections).
