...
The Harness exposes only logical rows in its model selector. Non-selectable rows are muted and rejected by the Host selection boundary. Context entries may carry `available: false` and an `unavailable_reason`; the selector displays those tiers muted. An ordinary request rejects an unavailable tier before provider I/O, while an explicit `bestTryContext: true` request may use its advertised private route. The flag is persisted with the model selection and request header, but never permits a context size the endpoint did not advertise.
...
An opted-in profile also probes the authenticated sibling `GET /running` endpoint while refreshing. Ready private context routes project an `active` state onto their logical row, and exact media rows project their own state. Runtime status is advisory: a missing or malformed status response leaves the catalog usable and reports no active rows.
