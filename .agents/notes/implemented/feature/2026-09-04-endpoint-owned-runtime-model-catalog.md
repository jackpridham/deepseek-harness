...
The Harness persists context and reasoning selections in the session request header, sends the matching private context route to the endpoint, and retains the logical model in Harness state and model provenance. Exact-model resolution preserves the catalog's reasoning labels, order, and default. Binary formats serialize one explicit disabled/enabled request; graded formats serialize each catalog `wire_value`. Qwen template effort transports send `chat_template_kwargs.enable_thinking` as the independent binary switch and top-level `reasoning_effort` as the selected grade. Exact-model resolution supplies the same effective context to admission, token-pressure, and compaction consumers. An unadvertised value, malformed block, duplicated effective request, or unsupported reasoning transport fails before provider I/O.
...
A real plugin composition test starts an authenticated mock endpoint, observes a model through `listModels()`, resolves its metadata, and streams with it without adding that id to configuration. It proves exact catalog reasoning names/order/default, the `id` to `wire_value` mapping for every advertised effort, the final Qwen template request body for Off/Low/Medium/Extra High, binary Off/On behavior, rejection of inert binary choices, absence of a selector without metadata, and ordinary/best-try context routing. Discovery and Host/UI tests pin parsing, bounded validation, persistence, non-selectable enforcement, active-state projection, and selection.
