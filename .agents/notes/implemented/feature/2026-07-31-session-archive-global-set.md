...
- UI: filtering is one extra arm in `tree.ts`'s `sessionVisible` predicate, with `deriveGroups`/`deriveFlat` taking an `archived` set parameter so all four surfaces (group loop, stray bucket, search, flat) share one source. The archive RPC remains available, but the Workspace browser's row action is the permanent deletion owned by the [direct Workspace and Session deletion decision](2026-09-04-direct-workspace-and-session-deletion.md).
...
Archived sessions have no viewing or unarchive surface; data and accounting slots stay intact, so a future restore is one UI surface plus one inverse RPC. The archive operation is currently API-only. Domain tests pin idempotence, unknown-id rejection, restart recovery, and the pre-field media default upgrade.
