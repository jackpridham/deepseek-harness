...
The agent loop measures the actual durable surface with its provisional header before dispatch, reserves the configured safety margin, gives the existing compaction provider one opportunity to reduce history, then sends the smaller effective cap or fails without dispatch. `output/budget` records that admission.
