...
The two upgrade routes exist only while `apiProxy` is available. Before registering them, connection also verifies that its buffered HTTP request cap can carry the active attachment service's aggregate image limit after base64 expansion. Deployment overlays must therefore keep the connection and attachment-local bundles in one compatibility closure: packaging a newer connection guard over an older attachment default can reject that optional lifecycle and leave both upgrade paths absent even though ordinary `/api` RPC remains healthy.
...
Webserver contract tests pin upgrade-pathname dispatch, duplicate-registration rejection, disposal, and teardown; connection real-network tests pin each WebSocket's trust check, open, schema envelope, frame order, stream error, and close cancellation; client tests also prove that downlinks create `ws:`/`wss:` URLs while unary calls and `respond` still use HTTP `fetch`. Deployment artifact tests pair the built connection and attachment-local bundles and activate `apiProxy` after connection startup, proving both upgrade routes appear and are removed again with service availability. The assembled keyless browser replay continues to cover Chromium, a real host, HTTP uplink, and the full WebSocket downlink chain.
