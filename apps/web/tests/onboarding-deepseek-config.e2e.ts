...
// credential is absent, the credential step uses the shipped modal chrome,
...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('never paints the takeover chrome on a configured reload, even with the settings join held open', async () => {
    ...
    // Regression pin for the reload flash: the credential step is satisfied,
    // yet it must load private facts before deciding not to show. Dialog chrome
    // lives inside the visible branch, so the deciding window paints and blocks
