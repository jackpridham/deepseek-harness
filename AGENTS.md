...
Docs accompany every code change: update affected English README and JSDoc contracts together. Chinese companion files are optional historical translations: do not create or update them unless the user explicitly requests translation work. Current-state prose, one physical line per paragraph, one home per fact, and word budgets live in [docs/AGENTS.md](docs/AGENTS.md).
