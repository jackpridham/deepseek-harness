...
describe('archived Agent Notes', () => {
  ...
  it('accepts an English note with optional historical companion artifacts', () => {
    expect(validateArchiveArtifacts(fixture())).toEqual([])
  })

  it('accepts an English-only archive entry', () => {
    const artifacts = fixture()
    artifacts.delete('process/2026-07-26-example.i18n.yaml')
    artifacts.delete('process/2026-07-26-example.zh.md')
    expect(validateArchiveArtifacts(artifacts)).toEqual([])
  })

  it('rejects companion artifacts without an English note and invalid English archive headers', () => {
    const companions = fixture()
    companions.delete('process/2026-07-26-example.md')
    expect(validateArchiveArtifacts(companions).join('\n')).toMatch(/archived companion files require/)
