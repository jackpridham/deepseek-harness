/** Pure archive-format, optional-companion, and immutable-manifest helpers. */
...
/** Compute the SHA-1 Git blob id used by optional translation sidecars. */
...
/** Validate the closed kind tree, English archive headers, and companion ownership. */
export function validateArchiveArtifacts(artifacts: ReadonlyMap<string, Buffer>): string[] {
  const errors: string[] = []
  const groups = new Map<string, ArchivedArtifacts>()
  ...
  for (const [key, group] of [...groups].sort(([left], [right]) => left.localeCompare(right))) {
    ...
    const { source } = group
    if (source === undefined) {
      errors.push(`${key}: archived companion files require ${sourcePath}`)
      continue
    }
