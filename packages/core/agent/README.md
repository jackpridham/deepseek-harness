...
Deployments register a versioned `SessionPolicy` through `ctx.agents.registerPolicy()`. A session's immutable `header.sessionPolicy` requires that provider whenever an agent enters the registry, including direct factory creation and persistence resume. Admission checks the provider's workspace, preset, and fork restrictions and synchronously calls `apply(agent)` after caller setup but before creation notifications. Missing providers and failed installers reject publication. Policy installers contribute through `agent.ctx`, so enforcement lasts for the agent scope; trusted host plugins are not sandboxed by this mechanism.

`policyIds()` lists available providers. `policyFor(sessionId)` returns the exact provider installed on a live agent, so unloading or replacing a provider cannot change that agent's reported assertions. Provider unload blocks new admission; existing agents retain their scoped enforcement. Providers must use a new identifier when guarantees change. The [policy ownership decision](../../../.agents/notes/implemented/architecture/2026-09-18-deployment-session-policies.md) explains the division between runtime admission and deployment behavior.
