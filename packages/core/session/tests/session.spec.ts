...
describe('Session', () => {
  ...
  it('rejects invalid scalar fields in an explicitly supplied header', () => {
    ...
    const cases: Array<{ header: unknown; error: RegExp }> = [
      ...
      { header: { ...base, sessionMode: 'advisory' }, error: /sessionMode is unsupported/ },
      { header: { ...base, sessionPolicy: '' }, error: /sessionPolicy must be a non-empty string/ },
      { header: { ...base, sessionPolicy: 1 }, error: /sessionPolicy must be a non-empty string/ },
