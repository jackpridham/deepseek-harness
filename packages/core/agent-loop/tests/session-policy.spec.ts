...
function policy(overrides: Partial<SessionPolicy> = {}): SessionPolicy {
  return {
    id, instructions: false, workspace: false, presets: false, fork: false, attestation: { locked: true },
...
describe('session policy admission', () => {
  ...
  it('rejects instructions before creating a restricted session', async () => {
    const ctx = await harness()
    ctx.agents.registerPolicy(policy())
    await expect(ctx.agents.create({
      sessionId,
      meta: { sessionPolicy: id },
      instructions: { version: 1 },
    })).rejects.toThrow('forbids instructions')
    expect(ctx.sessions.list()).toEqual([])
  })
