...
Before dispatch, a resolved context/output pair is measured against the complete request. The loop reads token-meter and compaction from the agent's isolated preset realm when one exists, otherwise from its ordinary scope; this keeps per-preset compaction reachable without duplicating the service. A bounded compaction attempt precedes any reduced effective output cap.
