...
- `ctx.tools.denyAllTools(): () => void` applies a scoped final capability boundary: schemas, lookup, and dispatch expose no inherited, scope-local, later-registered, or Code Mode `run_code` tool. It throws from a plain context because a process-global denial would mask every agent. Disposed with the calling fiber.
