...
Managed worker telemetry enriches model metadata when ready; an unavailable or transitioning worker does not remove its catalog entry or other models from the picker. Inference dispatch requires a fresh worker snapshot. When the chat requests different context, mode, or serving options, it submits the matching catalog route with automatic worker switching and the observed identity. The scheduler checks that identity, drains active requests, and enforces capacity protections before replacement; unavailable worker state still prevents dispatch. A stale saved identity alone does not reload a worker whose settings match. When that snapshot reports a scheduler transition, dispatch waits with cancellation support for readiness; if the transition ends with the worker stopped, the turn reports `WORKER_LOAD_FAILED` rather than silently starting another load. Catalog reads remain immediate, and a worker that was already stopped before dispatch retains normal cold-load behavior.
  ...
  config:
    providers:
      ...
      acme-gateway:
        ...
        # Make the endpoint's /models response authoritative for selectable
        # membership; configured entries become per-id overrides.
...
`modelsFromEndpoint: true` makes an OpenAI-compatible route's `GET /models` response authoritative for selectable membership. Model listing always refreshes it; resolution and streaming refresh on first use and when a requested id is not in the last observation. Existing `models` entries become optional per-id overrides for advertised entries, so a deployment can retain local capacities or compatibility fixes without copying the whole remote catalog. The route requires an explicit `baseURL` and can start without any local `models` entries. It registers with an empty catalog and performs no discovery request during startup; the first model listing or resolution fetches the endpoint. During that empty bootstrap, route-level `compat` is validated against the explicitly configured `api`. Endpoint failure fails the operation instead of silently serving stale membership. Listings may provide `name`, `context_length`/`context_window`, `max_tokens`, `capabilities.tools`, and the llama-swap `architecture.input_modalities` extension; missing fields retain the route fallbacks. A supplied `capabilities.tools` must be boolean. Tool support is advisory: `false` and omitted metadata remain visible to clients but never suppress offered tools or block provider dispatch.

Endpoint rows may publish `description`, `release_date` (a quoted `YYYY-MM-DD` string), and `url` (the HTTP(S) model card or download page). Discovery exposes them as `description`, `releaseDate`, and `url` in the shared Harness catalog; missing or null dates remain unknown. These fields describe the model and do not affect inference settings.

An endpoint row may additionally publish `context_windows: [{context_window, model, available?, unavailable_reason?}]`. The logical row remains the Harness model identity, while each bounded value maps to the exact endpoint-private model id that starts that runtime tier. `available` defaults true; a false tier stays visible but requires request-level `bestTryContext: true`, allowing a constrained host to attempt it and let llama-swap unload conflicting residents without making it an ordinary selection. Exact resolution makes the selected capacity authoritative for token pressure and compaction, and streaming substitutes the private id and selected context capacity at the provider boundary so pi-ai budgets output against the serving tier.

When a worker is already ready on a best-try tier, that active configuration is selectable and admitted; cold admission to the tier still requires `bestTryContext: true`.

The same row may publish `reasoning: {format, default_effort, efforts}`. That block is the sole reasoning authority for an endpoint-owned model: its labels, order, default, and wire spellings reach the exact-model selector unchanged, while a row without it exposes no reasoning selector even if local settings or pi-ai would otherwise infer one. Formats describe transport behavior rather than model families. Binary Qwen formats admit at most one disabled and one enabled choice; graded OpenAI, DeepSeek, OpenRouter, Together, ZAI, Qwen-effort, string-thinking, and Ant-Ling formats require request-distinct wire values. `qwen-chat-template-effort` sends the binary `chat_template_kwargs.enable_thinking` switch separately from the selected top-level `reasoning_effort`; plain `qwen-chat-template` remains binary. Malformed blocks, unsupported formats or effort ids, and choices a transport cannot distinguish fail catalog refresh. No local profile edit is required when the endpoint adds a model, proven-safe context tier, or verified reasoning choice.
...
The adapter exposes each configured route's models through `ctx.llm.listModels(provider)`. This is provider-neutral selector metadata read from the same pi-ai `Models` collection the request path uses, so discovery does not create a second model registry. Endpoint rows may set `selectable: false` to retain image/video runtime entries without offering them as conversation models. During an endpoint-owned refresh, the adapter also makes an authenticated best-effort `GET /running` beside the route's `/v1` surface; a logical model is `active` only when its id or one of its private context-route ids is reported `ready`. A failed or malformed runtime-status reply leaves every row inactive without hiding the otherwise valid model catalog. `ctx.llm.resolveModelInfo(provider, model)` performs that exact descriptor lookup once and returns its identity, context window, configured output cap, and selectable thinking levels, keeping authoritative metadata on the route-owning adapter rather than its consumers. A model's **configured** `maxTokens` becomes the seam's `defaultMaxTokens`, so a request that names no output cap carries the one the deployment chose; a value inherited from the installed catalog is the model's output *capability* and never becomes a request default on its own.

A non-endpoint model that carries reasoning metadata — from the installed catalog or from its entry's `reasoningEfforts` — exposes pi-ai's ordered `getSupportedThinkingLevels(model)` result. An endpoint-owned model instead exposes only its advertised reasoning block, preserving the endpoint's names and order exactly; provider wire spellings remain inside pi-ai's `thinkingLevelMap`.
...
The plugin offers `ctx.llm.registerModelDiscovery('llm-pi-ai', …)`, which answers "which models can this provider serve?" for a route a configuration surface is editing or drafting. Nothing is stored by that action, and its reply remains candidates the surface can adopt. Separately, a configured `modelsFromEndpoint` route uses the same bounded interrogation to build its current in-memory runtime catalog.
...
- **Endpoint-owned catalogs are request-time observations** — `modelsFromEndpoint` has no background poll or durable cache. Picker listing refreshes every time, while resolution and streaming reuse the current immutable snapshot until an unknown id requires another observation; endpoint failure is visible to the caller.
