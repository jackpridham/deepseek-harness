...
An endpoint row may additionally publish `context_windows: [{context_window, model}]`. The logical row remains the Harness model identity, while each bounded value maps to the exact endpoint-private model id that starts that runtime tier. Selectors expose only those values, exact resolution makes the selected capacity authoritative for token pressure and compaction, and streaming substitutes the private id only at the provider boundary.
