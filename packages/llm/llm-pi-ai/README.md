...
The same row may publish `reasoning: {format, default_effort, efforts}`. The currently supported endpoint format is `qwen-chat-template`; its bounded effort ids become exact-model selector choices and pi-ai maps `off`/a selected effort to `chat_template_kwargs.enable_thinking` false/true. Models without this block expose no selector. Unsupported formats or effort ids fail catalog refresh rather than becoming inert UI metadata. No local profile edit is required when the endpoint adds a model, proven-safe context tier, or verified reasoning choice.
