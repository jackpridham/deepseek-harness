...
The expanded brand row renders the single `sidebar.brand.name` slot; without an occupant, the shell shows `Vortex Harness` with the build's 7-character `DSH_CLIENT_COMMIT_HASH` badge. The collapsed rail uses the panel icon directly. A deployment package can replace the name without replacing the New Session control or rail geometry; declaration-aware `slots.inject()` lets such a package activate before or after the sidebar.
