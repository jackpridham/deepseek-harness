...
展开的品牌行渲染 single slot `sidebar.brand.name`；没有 occupant 时，外壳显示 `Vortex Harness`，并附带构建期 7 位 `DSH_CLIENT_COMMIT_HASH` 徽标。收起轨道直接使用面板图标。部署包可以替换名称，而无须替换 New Session 控件或轨道几何；声明感知的 `slots.inject()` 让这种包无论先于还是后于侧边栏激活都能生效。
