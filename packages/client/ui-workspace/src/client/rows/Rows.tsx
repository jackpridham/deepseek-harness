...
function pointRect([x, y]: readonly [x: number, y: number]): DOMRect {
...
export function ProjectRowItem({ group, onToggle, onCreate, actions, drag, home, t }: {
  ...
  const [menuAnchor, setMenuAnchor] = useState<readonly [x: number, y: number] | null>()
      ...
      <span className={css.rowActions}>
        {actions !== undefined && (
          <Menu
            ...
            portal
            closeOnPointerLeave
            {...(menuAnchor === undefined || menuAnchor === null
              ? {}
              : { getAnchorRect: () => pointRect(menuAnchor) })}
            anchor={(
              <button
                type="button"
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  setMenuAnchor(value => value === undefined ? null : undefined)
                }}
              >
 ...
 * @param props.onDelete - permanently delete a session by id.
...
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onDelete, drag, flat = false, t }: {
  ...
  /** Permanently delete this session (row menu action; commits without a dialog). */
