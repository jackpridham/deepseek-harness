...
Model selection plugin, browser half: TWO entries over ONE per-session directory owned by `ModelDirectoryResolver` (`ctx.modelDirectories`). For ordinary sessions, the `/model` popupSelect contribution (registered through `ctx.commandUi`) and the composer's named `conversation.input.model` seat both load the session's advisory directory through `session.models` and submit through `session.selectModel` via the same `ModelDirectory` instance. The composer exposes adjacent one-click Model, Context, and Effort selectors. Models stay provider-grouped; Context shows only the selected model's endpoint-advertised tiers. A host-constrained tier stays selectable, carries an orange warning with the endpoint's reason, and submits the explicit best-try override; selecting an ordinary tier again clears that override. For an advertised selection, Effort is always present: it shows that exact model's adapter-owned choices, or remains disabled with an explanatory tooltip when the model has none. Each menu opens into the larger viewport side, caps its height at the available edge, and scrolls long lists. Non-selectable rows retain legible muted text and a full-strength green dot when the Host reports them active. `/model` and model-row selections apply the model's advertised defaults.
