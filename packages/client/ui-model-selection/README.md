...
Model selection plugin, browser half: TWO entries over ONE per-session directory owned by `ModelDirectoryResolver` (`ctx.modelDirectories`). For ordinary sessions, the `/model` popupSelect contribution (registered through `ctx.commandUi`) and the composer's named `conversation.input.model` seat both load the session's advisory directory through `session.models` and submit through `session.selectModel` via the same `ModelDirectory` instance. The composer exposes adjacent one-click Model, Context, and Effort selectors. Models stay provider-grouped; Context shows the union of endpoint-advertised tiers, muting choices unavailable for the selected model; Effort shows that exact model's adapter-owned choices. Each menu opens into the larger viewport side and caps its height at the available edge. Non-selectable rows retain legible muted text and a full-strength green dot when the Host reports them active. `/model` and model-row selections apply the model's advertised defaults.

The Host-reported provider/model/context/reasoning `ModelSelection` is the single selection fact, but it is echoed only when the exact provider/model pair remains in the advertised groups; an absent catalog row leaves the routable selection intact while the trigger prompts `Select model`, no stale row is synthesized, and no Context or Effort row is shown until the user picks an advertised model. Directory loads and selections share a generation counter so an older response never overwrites a newer one; a connection reset drops every resident projection and repulls the Host-restored selection before display. Provider-local metadata failures list inline while usable groups stay selectable, and selection failures retain the prior selection and directory.
...
- **No arbitrary context input** — the composer displays the endpoint-advertised tier union but permits only tiers available for the selected model; a model without bounded choices leaves the Context selector absent.
