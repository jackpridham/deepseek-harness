...
The nullable `sessions.session_policy` column preserves the immutable deployment-policy identifier through reload. Schema 18 requires that column; older databases reject without migration. Agent admission requires the recorded provider before any resumed agent can run.
