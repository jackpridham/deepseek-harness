...
- **Output admission** — `compactForOutputBudget()` prices the exact upcoming header against the selected context tier and requested output reserve. It bypasses the normal pressure threshold, but retains the same bounded region and shrink rules.
