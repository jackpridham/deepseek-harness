...
export const rpcErrorSchema: z.ZodType<RpcError> = z.discriminatedUnion('code', [
  ...
  z.object({ code: z.literal('session-not-advisory'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('session-mode-conflict'), message: z.string(), details: z.object({ sessionId: z.string(), requestedMode: z.union([z.literal('ordinary'), z.literal('advisory')]), existingMode: z.union([z.literal('ordinary'), z.literal('advisory')]) }) }),
  z.object({ code: z.literal('advisory-session-invalid'), message: z.string(), details: z.object({}) }),
