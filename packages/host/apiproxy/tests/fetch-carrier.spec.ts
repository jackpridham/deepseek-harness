  ...
  return {
    ...
    host: {
      ...
      async listDirectory(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { path: '/w', home: '/w', crumbs: [{ name: '/', path: '/', hidden: false }], entries: [], canCreate: true, truncated: false } } }
      },
    ...
    workspace: {
      ...
      async deleteSession(request) {
        return {
          ...
          result: {
            ok: true,
            value: { deleted: true as const, deletedSessionIds: [request.payload.sessionId] },
          },
        }
      },
    },
...
describe('unary round trip (handler ⇄ client, no network)', () => {
  ...
  it('round-trips the browse listing and creation calls through the wire form', async () => {
    ...
    expect(listed.result).toEqual({
      ok: true,
      value: { path: '/w', home: '/w', crumbs: [{ name: '/', path: '/', hidden: false }], entries: [], canCreate: true, truncated: false },
    })
