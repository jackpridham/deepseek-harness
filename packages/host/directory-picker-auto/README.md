...
`config.defaultDirectory` chooses the browse interaction's first listed directory and defaults to the host account's `homedir()`. For example, a service may set `directory-picker` to `config: { defaultDirectory: '/srv/deepseek-harness/workspace' }`; the auto chooser forwards that value only when it mounts the browse backend. It does not restrict later navigation.
