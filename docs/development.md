...
The optional pairing merge driver derives a conflicted `.i18n.yaml` record from the confirmed ancestor, current, and other owner blobs when both language files use Git's default text strategy and merge cleanly. It serves explicitly maintained translation pairs only; ordinary English documentation work does not create or update pairing records. After an already-stopped optional-pair merge, run `pnpm run resolve-translation-pairing-conflicts`. See the [translation workflow](i18n/README.md#explicit-translation-work).
...
- `pre-merge-commit` validates frozen Agent Note archive artifacts when they change.
