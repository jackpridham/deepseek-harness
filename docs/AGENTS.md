...
| Generated reference: the per-page `cordis-surface` regions in [subsystems/](subsystems/README.md), the [Cordis core API + inherited tier](cordis-api/context.md), [tool-catalog](tool-catalog.md), [config-catalog](config-catalog.md), [persistence-catalog](persistence-catalog.md), [module-graph.md](module-graph.md) | Exhaustive English sources regenerated from source and freshness-gated | Hand edits to generated sources or regions |
...
- **English is the maintained documentation language.** Existing `.zh.md` files and `.i18n.yaml` records may remain as historical translations, but ordinary work does not create, update, re-record, or require them. Translate only when the user explicitly requests it ([optional workflow](i18n/README.md)).
